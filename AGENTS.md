...
- Перед тем как сказать «всё готово» — **самому проверить**.
- Запустить код, убедиться что нет ошибок, синтаксис корректен, поведение соответствует ожиданиям.
- Если можно запустить тест/генерацию — запустить.
